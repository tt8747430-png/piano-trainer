...
| **Down** (a key)              | A key sounding now (under Spotlight, struck last), held on a MIDI keyboard, or pressed by a pointer, drawn pressed; Name chord's shown chord is **lit**, not down | active, playing             |
...
| **Rail**                      | The dark fascia the keys hang from, holding ‹ ›, the Keyboard map and the settings button (`RailButton`)                                                          | toolbar, header             |
...
| **Spotlight**                 | On the explorers', Symbols' and a Piece's keyboards: only the keys **struck** last are down, and the other marks go quiet                                         | highlight, focus            |
