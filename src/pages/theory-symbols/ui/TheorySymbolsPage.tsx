...
export function TheorySymbolsPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <ReadingSheet />
      <Pinned>
        <LiveKeyboard label={t('common:keyboard')} range={MIDDLE_OCTAVES} className="h-32" />
      </Pinned>
      {CHORD_FAMILIES.map((family) => (
        <section
          key={family}
          aria-label={t(`theory:family.${family}`)}
          className="flex flex-col gap-3"
        >
          <h2 className="text-xl font-bold">{t(`theory:family.${family}`)}</h2>
